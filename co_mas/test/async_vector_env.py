...
def async_vector_env_test(async_vec_env: AsyncVectorParallelEnv, num_cycles=1000):
    """
    Test whether the SyncVectorParallelEnv can run an episode.
    """
    async_vec_env.reset(seed=42)
    # Test contents
    # 1. where the sub-environments reset correctly

    obs, info = async_vec_env.reset()
    should_be_reset = {env_id: len(_agents) == 0 for env_id, _agents in async_vec_env.agents.items()}

    logger.trace("agents:\n" + pformat(async_vec_env.agents))
    logger.trace("envs_have_agents:\n" + pformat(async_vec_env.envs_have_agents))
    logger.trace("info:\n" + pformat(info))
    for _ in tqdm(range(num_cycles)):
        ...
        envs_have_agents = async_vec_env.envs_have_agents
        ...
        try:
            ...
            if list(state.keys())[0] in async_vec_env.env_ids:
        ...
        obs, rew, terminated, truncated, info = async_vec_env.step(action)
        ...
        agents = async_vec_env.agents
        should_be_reset = {env_id: len(_agents) == 0 for env_id, _agents in agents.items()}
        # logger.trace(f"{pformat(should_be_reset)}\n==?\n{pformat(async_vec_env.unwrapped._autoreset_envs)}")
        # assert all(
        #     async_vec_env.unwrapped._autoreset_envs[env_id] == should_be_reset[env_id] for env_id in async_vec_env.env_ids
    ...
    logger.success("AsyncVectorParallelEnv Test Passed!")
